...
for k in range(num_steps):
    ...
    a = 2.0  # constant acceleration (input)
    ...
    # 2. Update
    # ???
    Kk = (P_pred @ H.T @ np.linalg.inv(H @ P_pred @ H.T + R)) * 0 #change the gain to just 0 to not consider measurements
    x_est =  x_pred + Kk*(z - H @ x_pred)
    P = (np.identity(2) - Kk @ H) @ P_pred
...
plt.plot(t, estimated_positions, 'r.', label='Estimated Position')
plt.xlabel('Time [s]')
plt.ylabel('Position')
plt.title('Pure Prediction Plot (Position Time Plot)')
plt.legend()
plt.grid(True)
plt.show()